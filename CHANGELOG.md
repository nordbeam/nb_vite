...
* **Phoenix 1.8 Colocated Hooks Support**
  * Automatic detection of Phoenix 1.8 from mix.exs dependencies
  * Configures Vite aliases for `phoenix-colocated/{app_name}` imports
  * Passes environment variables: `PHX_BUILD_PATH`, `PHX_APP_NAME`, and `PHX_VERSION`
  * Works seamlessly with Phoenix.LiveView.ColocatedHook script tags
  * Support for both development and production builds
  * Compatible with all package managers including Elixir-managed Bun

### Improvements

* Fixed Vite plugin import path to explicitly use `index.js`
* Enhanced installer to detect Phoenix version and configure accordingly
* Updated Bun integration to pass Phoenix-related environment variables
* Improved documentation for colocated hooks usage and compilation order
