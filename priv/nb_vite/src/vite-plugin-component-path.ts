 ...
 * ```
 */
export function componentPath(options: ComponentPathPluginOptions = {}): Plugin {
  const opts = {
    enabled: true,
    root: 'assets',
 ...
 */
function transformReactComponent(code: string, componentPath: string, id: string): { code: string; map: any } | null {
  try {
      ...
      VariableDeclarator(path: any) {
        ...
        if (t.isArrowFunctionExpression(init) || t.isFunctionExpression(init)) {
          ...
          if (t.isIdentifier(path.node.id)) {
            ...
            if (binding && binding.referencePaths.some((ref: any) => {
              return ref.findParent((p: any) => p.isExportDefaultDeclaration());
            })) {
              transformFunctionBody(init, attributeValue);
              hasModifications = true;
            }
 ...
 */
function transformFunctionBody(func: t.Function, componentPath: string): void {
 ...
 */
function addAttributeToJSXElement(element: t.JSXElement, componentPath: string): void {
  const openingElement = element.openingElement;

  // Check if attribute already exists
  const hasAttribute = openingElement.attributes.some(attr =>
    t.isJSXAttribute(attr) && t.isJSXIdentifier(attr.name) && attr.name.name === 'data-nb-component'
  );
  ...
  const attribute = t.jsxAttribute(
    t.jsxIdentifier('data-nb-component'),
 ...
 */
function transformVueComponent(code: string, componentPath: string): { code: string; map: null } | null {
