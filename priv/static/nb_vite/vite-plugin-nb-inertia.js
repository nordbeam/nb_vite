...
//#region src/vite-plugin-nb-inertia.ts
...
function nbInertia(options = {}) {
	...
	if (!opts.enabled) return { name: `${PLUGIN_NAME}-disabled` };
	let server = null;
	...
	function log(message) {
		if (opts.verbose) console.log(`${LOG_PREFIX} ${message}`);
	}
	/**
	...
	*/
	function resolvePageComponent(componentPath) {
		...
		if (fs.existsSync(colocatedExact)) return colocatedExact;
		const standaloneExact = path.resolve(resolvedStandaloneDir, componentPath);
		if (fs.existsSync(standaloneExact)) return standaloneExact;
		return null;
	}
	/**
	...
	*/
	function triggerExtraction() {
		...
		if (opts.extractUrl) fetch(opts.extractUrl, { method: "POST" }).then((res) => {
			isExtracting = false;
			if (res.ok) {
				log("Extraction via HTTP completed successfully");
				invalidatePageModules();
			} else console.error(`${LOG_PREFIX} Extraction HTTP request failed with status ${res.status}`);
		}).catch((err) => {
			isExtracting = false;
	...
	function triggerExtractionViaMix() {
		...
		child.on("close", (code) => {
			...
			if (code === 0) {
				...
				invalidatePageModules();
			} else console.error(`${LOG_PREFIX} Extraction failed with code ${code}`);
		});
		child.on("error", (err) => {
			isExtracting = false;
	...
	*/
	function invalidatePageModules() {
		...
		if (fs.existsSync(resolvedPagesDir)) invalidateDir(resolvedPagesDir);
		if (invalidatedCount > 0) log(`Invalidated ${invalidatedCount} module(s) in graph`);
		else {
		...
		function invalidateDir(dir) {
			...
			for (const entry of entries) {
				...
				if (entry.isDirectory()) invalidateDir(fullPath);
	...
	*/
	function matchesWatchPattern(filePath) {
		return opts.watchPaths.some((pattern) => {
	...
	return {
		...
		configResolved(resolvedConfig) {
			...
			resolvedPagesDir = path.isAbsolute(opts.pagesDir) ? opts.pagesDir : path.resolve(root, opts.pagesDir);
			resolvedStandaloneDir = path.isAbsolute(opts.standaloneDir) ? opts.standaloneDir : path.resolve(root, opts.standaloneDir);
			log(`Pages dir (colocated): ${resolvedPagesDir}`);
			log(`Pages dir (standalone): ${resolvedStandaloneDir}`);
		},
		resolveId(id) {
			if (!id.startsWith(PAGES_ALIAS)) return null;
			const resolved = resolvePageComponent(id.slice(7));
		...
		configureServer(devServer) {
			...
			if (fs.existsSync(resolvedPagesDir)) devServer.watcher.add(resolvedPagesDir);
