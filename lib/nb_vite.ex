defmodule NbVite do
  ...
  This is a pure Vite + Phoenix integration library. For Inertia.js support,
  see the separate nb_inertia package.
  """
  ...
  ## Examples
      ...
      <%= NbVite.vite_assets(["js/app.js", "js/admin.js"]) %>
  """
  ...
  ## Example
      ...
      <link rel="stylesheet" href={NbVite.asset_path("css/app.css")} />
  """
  def asset_path(path) do
    if dev_server_running?() do
      "#{vite_server_url()}/#{String.trim_leading(path, "/")}"
    else
      static_url = Application.get_env(:nb_vite, :static_url_path, "/")
  ...
  """
  def plugin_path do
    Path.join(:code.priv_dir(:nb_vite), "static/nb_vite")
  end
